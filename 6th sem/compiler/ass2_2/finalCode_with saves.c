...
void saveSyntaxTreeRecursive(struct syntaxNode *synRoot, FILE *fpsyntax)
{
	...
	if(synRoot==NULL)
		return;
	fprintf(fpsyntax,"%ld ",(long int)(synRoot));//node address
	fprintf(fpsyntax,"%c ",synRoot->data);//node data
	fprintf(fpsyntax,"%d ",synRoot->nullable);//node is nullable
	fprintf(fpsyntax,"%d ",synRoot->firstpos.poscount);//firstpos length
	for(i=0;i<synRoot->firstpos.poscount;i++)
		fprintf(fpsyntax,"%d,",synRoot->firstpos.poslist[i]);//firstpos csv
	fprintf(fpsyntax," %d ",synRoot->lastpos.poscount);//lastpos length
	for(i=0;i<synRoot->lastpos.poscount;i++)
		fprintf(fpsyntax,"%d,",synRoot->lastpos.poslist[i]);//lastpos csv

	fprintf(fpsyntax," %ld ",(long int)(synRoot->leftchild));//left child address
...
void printDFA(int symbolcount, int no_of_inputs, char *input, int _new_states)
{
...
void printMappedDFA(int new_states, int no_of_inputs, int symbolcount, char *inputs)
{
	int i, j, k;
...
int *calculateTransition(int *a, struct dfa_matrix *bfp, int symbolcount)
{
	int *b = (int *)malloc((symbolcount+1) * sizeof(int));
	int i, j;
...
int isNewState(struct dfa_matrix *s, int t, int *k, int symbolcount)
{
	int i, cnt = 0, j;
	for( i = 1; i <= symbolcount; i++ ){
		if( k[i] == 0 ) cnt++;
...
void mapDFA(int symbolcount, int no_of_inputs, int new_states)
{
	int i, j, k, t;
	char input = 'A';
...
char *markFinal(int *final_state_count, int new_states, int symbolcount)
{
	int i, j;
...
void saveDFA(int new_states, int no_of_inputs, int final_state_count, char *inputs, char *final_states)
{
	int i, j;
...
void calculateTransitionTable(struct syntaxNode *root, char *st, int symbolcount, struct followpos_matrix *fp, int no_of_inputs, int *ns, char *input)
{
	/*
		RULE:
	...
	*/
	struct dfa_matrix binFP[symbolcount+1], stack[MAX];//stack to keep track of all the marked states...
...
int main()
{
	...
	calculateTransitionTable(syntaxRoot, st, symbolcount, fp, uniqueSymbolCount-1, &new_states, uniqueSymbols);
	return 0;
