...
struct syntaxNode *Token2syntaxNode(struct tokenNode *token_n, int *flag, int *s_curr, int s_count)
{
	...
	struct syntaxNode *newNodePtr, *temp;
	&flag = 0;
	//cases:
...
{
	struct syntaxNode *synRoot=NULL, *synNew;
	int i, flag;

	//special handling if tokenCount is 1, skip cat

	//mini tree building loop
	for(i=0;i<(tokenarr->tokenCount)-1;i++)
	{
		...
		if(synRoot==NULL)
		{
			//take a node, store it in new->leftchild
			Token2syntaxNode(&(tokearr->list[i]),&flag,s_curr,s_count);

			//if flag=1||, call recursive littlearray1 and store it
				//if new->leftchild is still NULL, store it there only,

			  //otherwise store it in new->leftchild->leftchild

			//if flag=2, call recursive littleArray2 and store it in new->leftchild->rightchild

		}

		//store new pointer in root
		...
		//if flag=2, call recursive littleArray2 and store it in root->rightchild->rightchild
...
{
	//print recursivly with lvr rule with addresses
}
//syntax tree===========================================================================================================
...
int main()
{
  ...
  struct syntaxNode *syntaxRoot=NULL;
	...
	printf("The number of symbols is: %d",symbolcount);
